...
//                Log.e(TAG, "onResponse: " + action);
            ...
            if (obj.getInt("msgType") == 0) {
                ...
                if (MDDUtils.SearchAction.equals(action)) {
                    listSearchResult(call, dataList);
    ...
    private void listSearchResult(Call call, JSONArray dataList) throws Exception {
        String keyword = call.request().header("keyword");
        if (TextUtils.isEmpty(keyword) || !keyword.equals(MD5.encode(mSearchText))) {
            return;
        }
